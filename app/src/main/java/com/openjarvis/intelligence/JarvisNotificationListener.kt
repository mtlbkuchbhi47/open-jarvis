...
class JarvisNotificationListener : NotificationListenerService() {
    ...
    suspend fun replyToNotification(sender: String, packageName: String, reply: String): Boolean {
        return try {
            ...
            val replyTapped = service.tapByText("Reply") || service.tapByText("reply")
            if (!replyTapped) return false
            if (!service.typeText(reply)) return false
            service.tapByText("Send") || service.tapByText("send")
        } catch (e: Exception) {
            false
        }
    }
