...
class MCPManager(private val context: Context) {
    ...
    suspend fun callTool(serverId: String, toolName: String, args: JSONObject): String {
        val server = getServers().firstOrNull { it.id == serverId && it.enabled }
            ?: return "Error: MCP server not found or disabled"
        val client = clients.getOrPut(serverId) { MCPClient(server) }
        if (!client.isConnected() && !client.connect()) return "Error: MCP connection failed"
