...
class RiskyActionConfirmation(private val context: Context) {
    ...
    fun getRiskLevel(action: Action): RiskLevel {
        return when (action.action) {
            ...
            Action.TRADE_ORDER -> RiskLevel.HIGH
            Action.OPEN_APP -> {
    ...
    private fun buildSummary(action: Action): String {
        return when (action.action) {
            ...
            Action.TRADE_ORDER -> "LIVE TRADE: ${action.side?.uppercase()} ${action.qty ?: "?"} ${action.symbol ?: "?"}"
    ...
    private fun buildDetails(action: Action): List<String> {
        return listOf(
            if (action.action == Action.TRADE_ORDER) "Real-money broker order. Verify symbol, quantity and broker before proceeding." else "This action will interact with ${action.packageName}",
