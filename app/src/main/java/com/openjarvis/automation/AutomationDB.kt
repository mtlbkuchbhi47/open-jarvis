...
class AutomationWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            val agent = com.openjarvis.agent.AgentCore(applicationContext)
            val job = agent.executeTask(command)
            job.join()
            val success = agent.state.value is com.openjarvis.agent.AgentState.Done
