...
## Enhanced build (1.1.0)

This build closes several previously stubbed paths: all advertised Android actions now have execution handlers, accessibility gestures and screenshot capture use Android APIs, scheduled automations execute through `AgentCore`, notification listener registration is included, clipboard/MCP actions are wired, MCP JSON-RPC request handling is corrected, and a foreground hands-free voice loop with a “Hey Jarvis” wake-phrase approximation is included.

Some Android capabilities still depend on OS/user permissions (Accessibility, notification access, microphone, overlay) and OEM restrictions. No Android assistant can bypass those platform permissions.
...
MIT
## 1.2.0 Expanded Agent Features
See `docs/FEATURES_1_2_0.md` for trading/analysis, APK downloading, website generation, and science calculation capabilities. Trading is paper-first and cannot guarantee profit or eliminate loss.
